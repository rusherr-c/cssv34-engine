...
void CBaseGamesPage::RecalculateFilterString()
{
	...
	g_pVGuiLocalize->ConvertANSIToUnicode( "; ", spacerUnicode, sizeof( spacerUnicode ) );

	if (m_szGameFilter[0])
	{
		g_pVGuiLocalize->ConvertANSIToUnicode( m_szGameFilter, tempUnicode, iTempUnicodeSize );
	...
	if (m_iPingFilter)
	{
		...
		g_pVGuiLocalize->ConvertANSIToUnicode( " < ", tempUnicode, iTempUnicodeSize );
		wcscat( unicode, tempUnicode );
		g_pVGuiLocalize->ConvertANSIToUnicode(tmpBuf, tempUnicode, iTempUnicodeSize );
		wcscat( unicode, tempUnicode );	
		wcscat( unicode, spacerUnicode );
	}
	...
	if (m_szMapFilter[0])
	{
		g_pVGuiLocalize->ConvertANSIToUnicode( m_szMapFilter, tempUnicode, iTempUnicodeSize );
		wcscat( unicode, tempUnicode );
	}
